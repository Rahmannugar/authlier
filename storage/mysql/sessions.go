...
func (store *SessionStore) Create(ctx context.Context, record sessiontoken.Record) error {
	_, err := store.adapter.database.ExecContext(ctx, `INSERT INTO authlier_sessions
		...
		VALUES (?, ?, ?, ?, ?, ?, ?)`, record.ID, record.TokenHash[:], record.SubjectID,
...
func (store *SessionStore) FindByTokenHash(
	...
	return scanSession(store.adapter.database.QueryRowContext(ctx, `SELECT id, token_hash, subject_id,
...
func (store *SessionStore) ListBySubject(
	...
	rows, err := store.adapter.database.QueryContext(ctx, `SELECT id, token_hash, subject_id,
		created_at, expires_at, extended_at, revoked_at FROM authlier_sessions
...
func (store *SessionStore) Extend(
	...
	record, err := scanSession(tx.QueryRowContext(ctx, `SELECT id, token_hash, subject_id,
		created_at, expires_at, extended_at, revoked_at FROM authlier_sessions
		WHERE token_hash = ?`, tokenHash[:]))
	if err != nil {
		return record, err
...
func (store *SessionStore) Rotate(
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO authlier_sessions
		(id, token_hash, subject_id, created_at, expires_at, extended_at, revoked_at)
		VALUES (?, ?, ?, ?, ?, ?, ?)`, replacement.ID, replacement.TokenHash[:], replacement.SubjectID,
...
func (store *SessionStore) RevokeAll(
	...
	defer tx.Rollback()
	rows, err := tx.QueryContext(ctx, `SELECT id, token_hash, subject_id, created_at,
...
func (store *AccessSessionStore) CreateSession(
	...
	defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, `INSERT INTO authlier_access_sessions
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO authlier_refresh_tokens
		...
		VALUES (?, ?, ?, ?, ?, ?)`, refreshToken.TokenHash[:], refreshToken.SessionID,
...
func (store *AccessSessionStore) ResolveSession(
	...
	err := store.adapter.database.QueryRowContext(ctx, `SELECT id, subject_id, created_at, expires_at FROM authlier_access_sessions
...
func (store *AccessSessionStore) ListBySubject(
	...
	rows, err := store.adapter.database.QueryContext(ctx, `SELECT id, subject_id, created_at,
		expires_at, revoked_at FROM authlier_access_sessions WHERE subject_id = ?
...
func (store *AccessSessionStore) RevokeAll(
	...
	defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, `UPDATE authlier_refresh_tokens
		SET revoked_at = COALESCE(revoked_at, ?)
		WHERE session_id IN (SELECT id FROM authlier_access_sessions WHERE subject_id = ?)`,
