...
func (store *GoogleStore) ListIdentities(
	...
	rows, err := store.adapter.database.QueryContext(ctx, `SELECT provider_subject, email, linked_at
		FROM authlier_google_identities WHERE user_id = ? ORDER BY linked_at`, subjectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
